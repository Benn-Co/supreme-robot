...
let panorama;
  ...
  function autoStreetview(){   
    ...
    text = radius + " meters or less Street View near <b>" + localStorage.getItem("username") + "</b> location.";         
    showInContentWindow(text);
    sv.getPanorama({ location: new google.maps.LatLng(localStorage.getItem("latitude"),localStorage.getItem("longitude")), radius: radius }, processSVData);
    // Look for a nearby Street View panorama when the map is clicked.
    // getPanorama will return the nearest pano when the given
    // radius is 50 meters or less.
    map.addListener("click", (event) => {
      sv.getPanorama({ location: event.latLng, radius: radius }, processSVData);
      event_latLng = event.latLng;
  ...
  function processSVData(data, status) {
    if (status === "OK") {
      ...
      if (event_latLng != '') {
        text = radius + " meters or less Street View near <b>" + event_latLng + "</b>.";         
      } else {
  ...
  function showMap() {
    ...
    var locations = [
        ['' + localStorage.getItem("username") + '',localStorage.getItem("latitude"),localStorage.getItem("longitude"),0,],
    ];
